...
class APM(nn.Cell):
    ...
    def construct(self, x):
        ...
        neighbor_time_index = ops.cat([(ops.arange(0,t)+i).unsqueeze(0) for i in range(N) if i!=N//2], dim=0).t().flatten().long()

        # feature map registration
        semantic = self.semantic_mapping(x) # (b, c/16, t, h, w)
        x_norm = ops.normalize(semantic, p=2, dim=1) # (b, c/16, t, h, w)
        ...
        similarity = ops.matmul(x_norm_expand, neighbor_norm) * self.temperature # (b*t*2, h*w, h*w)
        similarity = ops.softmax(similarity, dim=-1) # (b*t*2, h*w, h*w)

        x_padding = self.padding(x)
        neighbor = x_padding[:, :, neighbor_time_index, :, :].permute(0, 2, 3, 4, 1).contiguous().view(-1, h*w, c)
        neighbor_new = ops.matmul(similarity, neighbor).view(b, t*(N-1), h, w, c).permute(0, 4, 1, 2, 3) # (b, c, t*2, h, w)
        ...
        x_offset = ops.zeros([b, c, N*t, h, w], dtype=x.data.dtype, device=x.device.type)
        x_index = ms.tensor([i for i in range(t*N) if i%N==N//2])
        neighbor_index = ms.tensor([i for i in range(t*N) if i%N!=N//2])
        x_offset[:, :, x_index, :, :] += x
...
class P3DA(nn.Cell):
    def __init__(self, conv2d, time_dim=3, time_stride=1, **kwargs):
        ...
        weight_2d = ops.eye(conv2d.out_channels).unsqueeze(2).unsqueeze(2)
        weight_3d = ops.zeros(*weight_2d.shape)
        weight_3d = weight_3d.unsqueeze(2).repeat(1, 1, time_dim, 1, 1)
        middle_idx = time_dim // 2
        weight_3d[:, :, middle_idx, :, :] = weight_2d
