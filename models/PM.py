...
def pcb_block(num_ftrs, num_stripes, local_conv_out_channels, feature_dim, avg=False):
    if avg:
        pooling_list = nn.CellList([nn.AdaptiveAvgPool2d(1) for _ in range(num_stripes)])
    else:
        pooling_list = nn.CellList([nn.AdaptiveMaxPool2d(1) for _ in range(num_stripes)])
    conv_list = nn.CellList([nn.Conv2d(num_ftrs, local_conv_out_channels, 1, bias=False) for _ in range(num_stripes)])
    batchnorm_list = nn.CellList([nn.BatchNorm2d(local_conv_out_channels) for _ in range(num_stripes)])
    relu_list = nn.CellList([nn.ReLU(inplace=True) for _ in range(num_stripes)])
    for m in conv_list:
        weight_init(m)
